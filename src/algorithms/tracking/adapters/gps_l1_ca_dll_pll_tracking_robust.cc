...
GpsL1CaDllPllTrackingrobust::GpsL1CaDllPllTrackingrobust(
    ...
    if (trk_params.item_type == "gr_complex")
        {
            ...
            tracking_ = dll_pll_veml_make_tracking_robust(trk_params);
        }
    else
        {
            item_size_ = sizeof(gr_complex);
...
void GpsL1CaDllPllTrackingrobust::stop_tracking()
{
    tracking_->stop_tracking();
}


void GpsL1CaDllPllTrackingrobust::start_tracking()
{
 ...
 */
void GpsL1CaDllPllTrackingrobust::set_channel(unsigned int channel)
{
    channel_ = channel;
...
void GpsL1CaDllPllTrackingrobust::set_gnss_synchro(Gnss_Synchro* p_gnss_synchro)
{
    tracking_->set_gnss_synchro(p_gnss_synchro);
}


void GpsL1CaDllPllTrackingrobust::connect(gr::top_block_sptr top_block)
{
    if (top_block)
...
void GpsL1CaDllPllTrackingrobust::disconnect(gr::top_block_sptr top_block)
{
    if (top_block)
        { /* top_block is not null */
        };
...
gr::basic_block_sptr GpsL1CaDllPllTrackingrobust::get_left_block()
{
    return tracking_;
}


gr::basic_block_sptr GpsL1CaDllPllTrackingrobust::get_right_block()
{
    return tracking_;
}
