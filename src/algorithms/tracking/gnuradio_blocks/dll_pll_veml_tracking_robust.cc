...
#include "dll_pll_veml_tracking_robust.h"
...
#else
        boost::bind(&dll_pll_veml_tracking_robust::msg_handler_telemetry_to_trk, this, _1));
#endif
...
void dll_pll_veml_tracking_robust::forecast(int noutput_items,
...
void dll_pll_veml_tracking_robust::msg_handler_telemetry_to_trk(const pmt::pmt_t &msg)
{
    try
        {
...
void dll_pll_veml_tracking_robust::start_tracking()
{
...
dll_pll_veml_tracking_robust::~dll_pll_veml_tracking_robust()
{
...
bool dll_pll_veml_tracking_robust::acquire_secondary()
{
...
bool dll_pll_veml_tracking_robust::cn0_and_tracking_lock_status(double coh_integration_time_s)
{
...
void dll_pll_veml_tracking_robust::run_dll_pll()
{
    // ################## PLL ##########################################################
    // PLL discriminator
    if (d_cloop)
...
void dll_pll_veml_tracking_robust::check_carrier_phase_coherent_initialization()
{
...
void dll_pll_veml_tracking_robust::clear_tracking_vars()
{
    std::fill_n(d_correlator_outs.begin(), d_n_correlator_taps, gr_complex(0.0, 0.0));
    if (trk_parameters.track_pilot)
        {
...
void dll_pll_veml_tracking_robust::update_tracking_vars()
{
...
void dll_pll_veml_tracking_robust::save_correlation_results()
{
    if (d_secondary)
        {
...
void dll_pll_veml_tracking_robust::log_data()
{
    if (d_dump)
        {
...
int32_t dll_pll_veml_tracking_robust::save_matfile()
{
    // READ DUMP FILE
...
void dll_pll_veml_tracking_robust::set_channel(uint32_t channel)
{
    gr::thread::scoped_lock l(d_setlock);
    d_channel = channel;
...
void dll_pll_veml_tracking_robust::set_gnss_synchro(Gnss_Synchro *p_gnss_synchro)
{
    gr::thread::scoped_lock l(d_setlock);
...
void dll_pll_veml_tracking_robust::stop_tracking()
{
    gr::thread::scoped_lock l(d_setlock);
    d_state = 0;
}
        ...
        case 2:  // Wide tracking and symbol synchronization
            {
             ...
             //   uint64_t d_buffer_count;
             //   volk_gnsssdr::vector<std::complex<float>> d_data_buffer;
            //    uint32_t d_consumed_samples;
            //    volk_gnsssdr::vector<std::complex<float>> d_input_signal1;
            //    d_consumed_samples=2000;
            //    d_data_buffer = volk_gnsssdr::vector<std::complex<float>>(d_consumed_samples);
            //    d_input_signal1 = volk_gnsssdr::vector<std::complex<float>>(d_consumed_samples);
           //     d_buffer_count = 0U;
           //     memcpy(&d_data_buffer, in, sizeof(gr_complex) * d_consumed_samples);
           //     memcpy(d_input_signal1.data(), d_data_buffer.data(), d_consumed_samples * sizeof(gr_complex));
           //     std::cout<<d_input_signal1[12].real()<<"real"<<d_input_signal1[12].imag()<<"end for tracking"<<d_consumed_samples * sizeof(gr_complex)<<std::endl;   
